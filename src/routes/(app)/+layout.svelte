<script lang="ts">
	...
	onMount(() => {
		...
		window.addEventListener(
			"open-message-stats",
			handleOpenStats as EventListener,
		);
		...
		const unsubscribeBackgroundComplete =
			streamingStore.onBackgroundComplete((chatId, chatName) => {
		...
		if (interruptedChats.length > 0) {
			interruptedChats.forEach(({ chatId, state }) => {
				...
				toastStore.show(
					`"${state.chatName}" was interrupted by page refresh. Partial content saved.`,
					"info",
				);
	...
	let mobileMenuOpen = $derived($uiStore.mobileMenuOpen);
...
>
	...
	{/if}
	<aside
		class="sidebar-left overflow-y-auto overflow-x-hidden transition-all duration-300 relative {leftSidebarCollapsed
			? ''
	...
	>
		...
		{#if !leftSidebarCollapsed}
			<div
				class="resize-handle absolute right-0 top-0 bottom-0 w-1 cursor-col-resize hover:bg-[#4299e1] transition-colors z-50 md:block hidden"
	...
	<main class="main-content overflow-hidden flex flex-col min-w-0">
		{@render children()}
	</main>
</div>
