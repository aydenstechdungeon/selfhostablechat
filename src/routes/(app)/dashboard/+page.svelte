<script lang="ts">
	...
	// LayerChart imports
	import { Chart, Svg, Axis, Bars, Area } from "layerchart";
	...
	let activeTab = $state<"overview" | "models" | "costs" | "trends">(
		"overview",
	);
...
<div class="dashboard-page px-8 py-6 overflow-y-auto h-full">
	<div class="max-w-7xl mx-auto">
		<!-- Header -->
		<div
			class="dashboard-header flex flex-wrap items-center justify-between gap-4 mb-8"
		>
		...
		<div class="flex gap-2 mb-6 border-b" style:border-color={borderColor}>
			{#each [{ id: "overview", label: "Overview", icon: BarChart3 }, { id: "models", label: "Models", icon: Brain }, { id: "costs", label: "Costs", icon: DollarSign }, { id: "trends", label: "Trends", icon: TrendingUp }] as tab}
				...
				<button
					...
					style:border-color={activeTab === tabItem.id
						? "#4299e1"
						: "transparent"}
					style:color={activeTab === tabItem.id
						? "#4299e1"
						: textSecondary}
		...
		{#if isLoading}
			...
			>
				<div class="text-center">
					<div
						class="animate-spin rounded-full h-12 w-12 border-b-2 border-[#4299e1] mx-auto mb-4"
					></div>
		...
		{:else if stats}
			{#if activeTab === "overview"}
				...
				<div class="space-y-6" transition:fade={{ duration: 200 }}>
					...
					>
						<div
							class="stat-card p-5 rounded-xl border"
							style:background-color={cardBg}
							style:border-color={borderColor}
						>
							<div class="flex items-center gap-3 mb-3">
								...
								<span
									...
									style:color={textSecondary}>Total Cost</span
								>
							</div>
							<div
						...
						>
							<div class="flex items-center gap-3 mb-3">
								...
								<span
									class="text-xs uppercase tracking-wide"
									style:color={textSecondary}>Messages</span
								>
							</div>
							<div
								class="text-3xl font-bold mb-1"
					...
					<div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
						<!-- Cost Over Time -->
						<div
							class="chart-card p-5 rounded-xl border"
							style:background-color={cardBg}
							style:border-color={borderColor}
						>
							<h3
						...
						<!-- Messages Per Model -->
						<div
							class="chart-card p-5 rounded-xl border lg:col-span-2"
							style:background-color={cardBg}
							style:border-color={borderColor}
						>
							<h3
					...
					<div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
						...
						>
							...
							<div class="space-y-3">
								...
								>
									<span style:color={textSecondary}
										>Total Custom Models</span
									>
									<span
										class="font-semibold"
										style:color={textPrimary}
										>{customModels.models.length}</span
									>
								</div>
								<div
									class="flex justify-between items-center p-3 rounded-lg"
				...
				<div class="space-y-6" transition:fade={{ duration: 200 }}>
					...
					>
						...
						<p style:color={textSecondary}>
							Coming soon: Detailed trend analysis with predictive
							insights
						</p>
					</div>
				</div>
