...
>
	{#if !hasApiKey}
		<div
			class="api-key-banner px-3 md:px-4 py-2.5 md:py-3 border-b"
			style:background-color={bgSecondary}
			style:border-color={borderColor}
		>
			<div
				class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 sm:gap-4 max-w-4xl mx-auto"
			>
				<div class="flex items-center gap-2 md:gap-3">
					<AlertTriangle
						size={18}
						color={alertColor}
						class="flex-shrink-0 md:size-[20px]"
					/>
					<div>
						<p
				...
				<button
					...
					class="w-full sm:w-auto px-3 md:px-4 py-1.5 md:py-2 rounded-lg bg-[#4299e1] text-white text-xs md:text-sm font-medium hover:bg-[#3182ce] transition-colors"
				>
					Add API Key
				</button>
			</div>
	...
	{#if isDragging}
		<div
			class="absolute inset-0 z-50 flex items-center justify-center pointer-events-none p-4"
			style:background-color={theme === "light"
				? "rgba(66, 153, 225, 0.15)"
				: "rgba(66, 153, 225, 0.25)"}
		>
			<div
				class="flex flex-col items-center gap-3 md:gap-4 p-8 md:p-12 rounded-2xl border-4 border-dashed transform scale-110"
				style:background-color={theme === "light"
			...
			>
				<div
					class="p-4 md:p-6 rounded-full"
					style:background-color="rgba(66, 153, 225, 0.2)"
				>
					<Upload size={48} color="#4299e1" class="md:size-[64px]" />
				</div>
				<p
				...
				<p
					class="text-xs md:text-sm opacity-70 text-center"
