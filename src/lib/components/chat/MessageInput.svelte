<script lang="ts">
	...
	$effect(() => {
		if (
			isCreatingNewChat &&
			submittingChatId &&
			$page.params.id === submittingChatId
		) {
	...
	$effect(() => {
		...
		const onDifferentChat =
			$page.params.id && $page.params.id !== submittingChatId;
	...
	let isImageGenModel = $derived(
		!autoMode && selectedModel && isImageGenerationModel(selectedModel),
	);

	async function handleSubmit() {
		if ((!message.trim() && attachedFiles.length === 0) || isSubmitting)
			return;
		...
		try {
			...
			const isStillOnSameChat =
				!currentPageChatId ||
				currentPageChatId === submissionTargetChatId ||
				currentPageChatId === submittingChatId;
	...
	function processFiles(files: globalThis.File[]) {
		for (const file of files) {
			...
			if (
				!ALL_SUPPORTED_TYPES.includes(file.type as any) &&
			...
			if (file.type.startsWith("image/") && !supportsImages) {
				toastStore.show(
					"Current model does not support images. Switch to a vision-capable model.",
					"error",
				);
				continue;
			}
...
<div
	class="message-input-container flex-shrink-0 transition-colors duration-200 relative"
	style:background-color={bgColor}
	style:border-top="1px solid {border}"
>
	<div class="max-w-4xl mx-auto px-3 md:px-6 py-3 md:py-4">
				...
				{#each attachedFiles as file, index}
					{#if file.type === "image"}
						<div
							...
							onkeydown={(e) =>
								e.key === "Enter" &&
								openImageModal(file.url, file.name)}
						>
							...
							<div
								class="absolute inset-0 bg-black/0 group-hover:bg-black/20 transition-colors flex items-center justify-center rounded-lg"
							>
								<span
		...
		<div class="relative flex items-stretch gap-2 md:gap-3">
			<div class="flex-1 relative">
				<button
					class="absolute left-3 md:left-4 top-3 p-1 rounded-lg transition-all duration-200 hover:opacity-70 hover:scale-110 active:scale-95 z-10"
					style:color={accentColor}
					onclick={openFilePicker}
					title="Attach files"
				>
					<Paperclip size={16} class="md:size-[18px]" />
				</button>
				<textarea
					class="w-full pl-10 md:pl-12 pr-3 md:pr-4 py-2.5 md:py-3 text-sm md:text-base rounded-xl border resize-none focus:outline-none transition-all duration-200 focus:ring-2 focus:ring-opacity-50"
					style:background-color={inputBg}
					style:border-color={border}
					style:color={text}
					...
					style="min-height: 44px; max-height: 320px; field-sizing: content; word-break: break-word; overflow-wrap: anywhere;"
				></textarea>
				{#if isImageGenModel}
					<div
						class="absolute right-3 top-3 flex items-center gap-1 text-xs"
						style:color={accentColor}
					>
						<Sparkles size={12} />
						<span>Image Gen</span>
			...
			{#if isStreamingState}
				<button
					class="flex-shrink-0 w-11 h-11 md:w-12 md:h-12 rounded-lg text-white transition-all duration-300 ease-in-out flex items-center justify-center hover:scale-105 active:scale-95 hover:shadow-lg animate-pulse"
				...
				>
					<Square
						size={14}
						fill="currentColor"
						class="transition-transform duration-200 md:size-[16px]"
					/>
				</button>
			{:else}
				<button
					class="flex-shrink-0 w-11 h-11 md:w-12 md:h-12 rounded-lg text-white transition-all duration-300 ease-in-out disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center hover:scale-105 active:scale-95 hover:shadow-lg"
				...
				>
					<Send
						size={14}
						class="transition-transform duration-200 md:size-[16px] {isSubmitting
							? 'animate-pulse'
							: ''}"
					/>
				</button>
