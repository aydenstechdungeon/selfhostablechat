<script lang="ts">
	...
	let { message, streaming = false, siblings = [], currentIndex = 0 }: Props = $props();
	
	// Detect if this message is currently being streamed into
	// (streaming is active and message has empty content or no stats yet)
	let isBeingStreamed = $derived($isStreaming && message.role === 'assistant' && 
	...
	async function saveEdit() {
		...
		try {
			...
			isEditing = false;
			toastStore.show('Message updated and response regenerated', 'success');
		} catch (error) {
	...
	$effect(() => {
		...
		return () => {
			window.removeEventListener('open-image-modal', handleOpenImageModal);
		};
	});
	...
	$effect(() => {
		...
		const handleImageClick = (e: MouseEvent) => {
			const button = (e.target as HTMLElement).closest('.image-preview-button');
			if (button) {
...
<div class="message-bubble group flex gap-3 animate-fade-in-up {message.role === 'user' ? 'flex-row-reverse' : ''}">
	<div class="message-avatar w-9 h-9 rounded-full flex items-center justify-center flex-shrink-0 transition-transform duration-200 hover:scale-110 {message.role === 'user' ? 'bg-[#4299e1]' : 'bg-[#2d3748]'}">
		{#if message.role === 'user'}
			<User size={18} class="text-white" />
		{:else}
			<Bot size={18} class="text-[#4299e1]" />
		{/if}
	</div>

	<div class="message-content max-w-[70%] min-w-[200px] rounded-2xl px-4 py-3 transition-all duration-200 hover:shadow-lg {message.role === 'user' ? 'bg-[#4299e1] text-white' : 'bg-[#1a1f2e] text-[#e2e8f0] border border-[#2d3748]'}">
		<div class="message-header flex items-center justify-between gap-4 mb-2 {message.role === 'user' ? 'opacity-80' : 'opacity-60'}">
			<span class="text-xs font-medium flex items-center gap-2">
				{message.role === 'user' ? 'You' : 'Assistant'}
				{#if message.model}
					<span class="px-1.5 py-0.5 rounded text-[10px] bg-white/20">{message.model}</span>
				{/if}
				{#if message.isPartial}
					<span title="Generation was interrupted - partial response saved" class="flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] bg-amber-500/20 text-amber-300">
						<AlertCircle size={10} />
						<span>Partial</span>
					</span>
				{/if}
		...
		<div class="message-body">
					...
					{#each message.media as media}
						{#if media.type === 'image'}
							<button
								...
								onclick={() => openImageModal(media.url, 'Attached image')}
							>
							<img 
								src={media.url} 
								alt=""
			...
			{:else}
				<div class="text-content text-sm leading-relaxed break-words min-h-[1.5em]" bind:this={contentElement}>
					...
					{/if}
					{#if showStreaming}
						<span class="streaming-cursor inline-block w-0.5 h-4 bg-current ml-0.5 animate-pulse"></span>
					{/if}
				</div>
...
<style>
	...
	:global(.text-content ul), :global(.text-content ol) {
		margin: 0.5rem 0;
		padding-left: 1.5rem;
	}
	...
	:global(.text-content h1), :global(.text-content h2), :global(.text-content h3),
	:global(.text-content h4), :global(.text-content h5), :global(.text-content h6) {
		margin: 1rem 0 0.5rem 0;
		font-weight: 600;
	}
	
	:global(.text-content h1) { font-size: 1.25rem; }
	:global(.text-content h2) { font-size: 1.125rem; }
	:global(.text-content h3) { font-size: 1rem; }
	...
	:global(.text-content th), :global(.text-content td) {
