<script lang="ts">
	...
	function inferStepType(title: string): ReasoningStep["type"] {
		...
		if (lower.includes("conclusion") || lower.includes("final"))
			return "conclusion";
		if (lower.includes("plan") || lower.includes("approach"))
			return "planning";
		if (lower.includes("reflect") || lower.includes("review"))
			return "reflection";
		if (lower.includes("deduc") || lower.includes("infer"))
			return "deduction";
		return "analysis";
	}
	...
	>
		...
		<div class="flex-1 text-left">
			<div class="flex items-center gap-2">
				...
				<span
					class="text-xs px-2 py-0.5 rounded-full bg-purple-500/20 text-purple-500"
				>
	...
	{#if isExpanded}
		...
		>
			{#each parsedSteps() as step, index}
				...
				<div
					class="reasoning-step rounded-lg border overflow-hidden"
					style:border-color={borderColor}
					class:active={activeStep === index}
				>
					<button
						class="step-header w-full flex items-center gap-3 p-3 transition-colors"
						style:background-color={activeStep === index
							? `${stepColor}15`
							: bgColor}
						onclick={() =>
					...
					>
						<div
							class="w-6 h-6 rounded flex items-center justify-center flex-shrink-0 text-xs font-bold text-white"
						...
						<ChevronRight
							size={16}
							color={textSecondary}
							class="transition-transform duration-200 {activeStep ===
							index
								? 'rotate-90'
								: ''}"
						/>
