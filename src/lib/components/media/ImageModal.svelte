<script lang="ts">
	...
	async function downloadImage(url: string = imageUrl, format?: string) {
		try {
			...
			if (url.startsWith("data:")) {
				...
				a.download =
					alt.replace(/[^a-z0-9]/gi, "_").toLowerCase() + "." + ext;
			...
			a.href = blobUrl;
			a.download =
				alt.replace(/[^a-z0-9]/gi, "_").toLowerCase() +
				"." +
				(format || "png");
			document.body.appendChild(a);
			a.click();
	...
	async function handleConvert() {
		...
		try {
			...
			toastStore.show(
				`Image converted to ${selectedFormat.toUpperCase()}`,
				"success",
			);
		} catch (error) {
...
>
	...
	<!-- Controls -->
	<div
		class="absolute bottom-4 left-1/2 -translate-x-1/2 flex items-center gap-2 p-2 rounded-full bg-white/10 backdrop-blur-sm"
	>
		<button
		...
		>
			<ZoomOut
				size={20}
				class="text-white {scale <= 0.5 ? 'opacity-50' : ''}"
			/>
		</button>
		...
		>
			<ZoomIn
				size={20}
				class="text-white {scale >= 3 ? 'opacity-50' : ''}"
			/>
		</button>
		<div class="w-px h-6 bg-white/20 mx-1"></div>
		<button
			class="p-2 rounded-full hover:bg-white/20 transition-colors {scale !==
				1 ||
			panX !== 0 ||
			panY !== 0
				? 'bg-white/20'
		...
		>
			<Move
				size={20}
				class="text-white {scale === 1 && panX === 0 && panY === 0
					? 'opacity-50'
					: ''}"
			/>
		</button>
		...
		<button
			class="p-2 rounded-full hover:bg-white/20 transition-colors {showConversionPanel
				? 'bg-white/30'
				: ''}"
	...
	{#if showConversionPanel}
		...
		>
			...
			<div class="mb-4">
				<p
					class="text-xs opacity-70 mb-2 block"
					style:color={panelText}
				>
					Target Format
				</p>
				<div class="grid grid-cols-2 gap-2">
					{#each IMAGE_CONVERSION_FORMATS as format}
						<button
							...
							style:background-color={selectedFormat ===
							format.format
								? "#4299e1"
								: "transparent"}
							style:color={selectedFormat === format.format
								? "#ffffff"
								: panelText}
							style:border-color={selectedFormat === format.format
								? "#4299e1"
								: panelBorder}
			...
			{#if selectedFormat === "jpeg" || selectedFormat === "webp"}
				<div class="mb-4">
					<label
						for="quality-slider"
						class="text-xs opacity-70 mb-2 block flex justify-between"
						style:color={panelText}
					>
						<span>Quality</span>
						<span>{quality}%</span>
					...
					<input
						...
						style:background="linear-gradient(to right, #4299e1 0%,
						#4299e1 {quality}%, {panelBorder}
						{quality}%, {panelBorder} 100%)"
					/>
				</div>
			{/if}
			...
			<!-- Size Comparison -->
			<div
				class="mb-4 p-3 rounded-lg text-xs"
	...
	>
		...
		<img
			...
			style:transform="scale({scale}) translate({panX / scale}px, {panY /
				scale}px)"
