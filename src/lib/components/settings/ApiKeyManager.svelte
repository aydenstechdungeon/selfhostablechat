...
<div class="api-keys-manager space-y-6">
	<div class="flex items-start gap-3 p-4 rounded-lg border" style:border-color={border} style:background-color={inputBg}>
		<div class="w-5 h-5 mt-0.5">
			<SiOpenstreetmap size={20} color="#4299e1" />
		</div>
		<div class="flex-1">
			<h3 class="font-semibold mb-1" style:color={textPrimary}>OpenRouter API Key</h3>
			<p class="text-sm" style:color={textSecondary}>
				Your API key is stored locally in your browser. Get your key from 
				<a href="https://openrouter.ai" target="_blank" rel="noopener" class="text-[#4299e1] hover:underline">openrouter.ai</a>
			</p>
		</div>
	</div>
	...
	<div class="space-y-4">
		<div class="setting-group">
			<label for="apiKey" class="block mb-2 text-sm font-semibold" style:color={textPrimary}>
				API Key
			</label>
			<div class="relative">
				<input
					...
					placeholder={store.apiKey ? '••••••••••••••••' : 'sk-or-v1-...'}
		...
		{#if statusMessage}
			<div 
				...
				style:background-color={saveStatus === 'error' ? 'rgba(220, 38, 38, 0.1)' : 'rgba(34, 197, 94, 0.1)'}
	...
	<div class="status-indicators space-y-2">
		<div class="flex items-center gap-2 text-sm">
			...
			<span style:color={textSecondary}>
				API Key: {store.apiKey ? 'Configured' : 'Not set'}
			</span>
		</div>
	</div>
	
	<div class="info-box p-4 rounded-lg border" style:border-color={border} style:background-color={inputBg}>
		<h4 class="font-semibold mb-2 text-sm" style:color={textPrimary}>Privacy Information</h4>
