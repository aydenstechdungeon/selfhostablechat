<script lang="ts">
	...
	let filteredModels = $derived(() => {
		...
		if (filterBrand !== "all") {
			models = models.filter(
				(m) => m.brand.toLowerCase() === filterBrand.toLowerCase(),
			);
		}
	...
	let visibleModels = $derived(() =>
		filteredModels().filter((m) => !m.isHidden),
	);
	let hiddenModelsList = $derived(() =>
		filteredModels().filter((m) => m.isHidden),
	);
	...
	let stats = $derived(() => {
		...
		return {
			...
			autoSelectable: visible.filter((m) =>
				"isAutoSelectable" in m ? m.isAutoSelectable !== false : true,
			).length,
	...
	function handleSave() {
		...
		const modelData: CustomModel = {
			...
			supportsImages: formData.capabilities?.includes("vision") || false,
			isRecommended: formData.isRecommended || false,
			isAutoSelectable: formData.isAutoSelectable !== false,
			isCustom: true,
			provider: formData.provider || "openrouter",
		};
	...
	function toggleRecommended(model: CustomModel) {
		if (model.isCustom) {
			customModelsStore.updateModel(model.id, {
	...
	function toggleAutoSelectable(model: CustomModel) {
		if (model.isCustom) {
			const current =
				"isAutoSelectable" in model ? model.isAutoSelectable : true;
			customModelsStore.updateModel(model.id, {
				isAutoSelectable: !current,
			});
		}
	}
...
<div class="model-manager space-y-6">
	...
	{#if visibleModels().length > 0}
		<div class="models-section">
			...
			<div
				class="models-list space-y-2 max-h-[400px] overflow-y-auto"
			...
			>
				{#each visibleModels() as model}
					...
					{@const isSvgIcon =
						DisplayIcon === DeepSeekLogo ||
						DisplayIcon === PerplexityAILogo ||
						DisplayIcon === XAIGrokLogo ||
						DisplayIcon === KimiLogo ||
						DisplayIcon === QwenLogo ||
						DisplayIcon === GrokLogo ||
						DisplayIcon === MiniMaxLogo}
					{@const isCustom = model.isCustom}
					{@const isAutoSelectable =
						"isAutoSelectable" in model
							? model.isAutoSelectable
							: true}

					<div
						class="model-item flex items-center gap-3 p-3 rounded-lg border transition-all hover:translate-x-1"
						style:border-color={border}
					...
					>
						...
						<!-- Brand/Model Icon -->
						<div
							class="w-10 h-10 rounded-lg flex items-center justify-center text-white flex-shrink-0 overflow-hidden"
						...
						<div class="flex-1 min-w-0">
							<div class="flex items-center gap-2">
								<span
									...
									style:color={textPrimary}
									>{model.displayName || model.name}</span
								>
								...
								{#if isCustom}
									<span
										class="text-[10px] px-1.5 py-0.5 rounded bg-blue-500/20 text-blue-500 font-medium"
										>Custom</span
									>
								{:else}
									<span
										class="text-[10px] px-1.5 py-0.5 rounded bg-gray-500/20 text-gray-500"
										>Built-in</span
									>
								{/if}
							</div>
							...
							>
								...
								<span class="text-green-500"
									>${model.pricePer1M.input.toFixed(2)} / ${model.pricePer1M.output.toFixed(
										2,
									)} per 1M</span
								>
						...
						<div class="flex items-center gap-1">
							...
							<button
								...
								style:background-color={model.isRecommended
									? "rgba(234, 179, 8, 0.2)"
									: "transparent"}
								onclick={() =>
									isCustom &&
									toggleRecommended(model as CustomModel)}
								title={model.isRecommended
							...
							<button
								class="p-2 rounded-lg transition-colors"
								style:background-color={isAutoSelectable
									? "rgba(34, 197, 94, 0.2)"
									: "transparent"}
								onclick={() =>
									isCustom &&
									toggleAutoSelectable(model as CustomModel)}
								title={isAutoSelectable
									? "Remove from auto mode"
									: "Add to auto mode"}
								disabled={!isCustom}
								class:opacity-50={!isCustom}
							>
								<Sparkles
									size={16}
									class={isAutoSelectable
										? "text-green-500"
										: ""}
									color={isAutoSelectable
										? undefined
										: textSecondary}
								/>
							</button>
							...
							{#if !isCustom}
								<button
									...
									onclick={() =>
										toggleModelVisibility(model.id, false)}
									title="Hide model"
								>
							...
							{#if isCustom}
								<button
									...
									onclick={() =>
										openEditModal(model as CustomModel)}
									title="Edit model"
								>
								...
								<button
									...
									onclick={() =>
										confirmDelete(model as CustomModel)}
									title="Delete model"
								>
	...
	{#if hiddenModelsList().length > 0 && filterVisibility !== "visible"}
		<div class="models-section mt-6">
			...
			{#if showHiddenModels}
				<div
					class="models-list space-y-2 max-h-[300px] overflow-y-auto"
					style="scrollbar-width: thin;"
				...
				>
					{#each hiddenModelsList() as model}
						...
						<div
							class="model-item flex items-center gap-3 p-3 rounded-lg border opacity-60"
							style:border-color={border}
							style:background-color={inputBg}
						>
							<!-- Brand/Model Icon -->
							<div
								class="w-10 h-10 rounded-lg flex items-center justify-center text-white flex-shrink-0 grayscale overflow-hidden"
							...
							<button
								...
								onclick={() =>
									toggleModelVisibility(model.id, true)}
							>
								Show Model
							</button>
						</div>
	...
	{#if filteredModels().length === 0}
		<div class="text-center py-12" style:color={textSecondary}>
			...
			<p class="text-sm">
				Try adjusting your filters or add a custom model
			</p>
		</div>
	{/if}
	...
	<!-- Legend -->
	<div
		class="flex flex-wrap items-center gap-4 text-xs pt-4 border-t"
		style:border-color={border}
		style:color={textSecondary}
	>
		<div class="flex items-center gap-1">
...
{#if showAddModal || showEditModal}
	...
	>
		<div
			class="w-full max-w-2xl rounded-xl border shadow-2xl max-h-[90vh] overflow-y-auto"
			style:background-color={contentBg}
			style:border-color={border}
		>
			<div
			...
			<div class="p-6 space-y-4">
				...
				<div class="setting-group">
					<label
						...
						style:color={textPrimary}>Model ID *</label
					>
					<input
						id="model-id"
						type="text"
					...
					<p class="text-xs mt-1" style:color={textSecondary}>
						Format: provider/model-name (cannot be changed later)
					</p>
				</div>
				...
				<div class="setting-group">
					<label
						for="model-name"
						class="block text-sm font-medium mb-2"
						style:color={textPrimary}>Display Name *</label
					>
					<input
						id="model-name"
						type="text"
				...
				<div class="grid grid-cols-2 gap-4">
					<div class="setting-group">
						<label
							for="model-brand"
							class="block text-sm font-medium mb-2"
							style:color={textPrimary}>Brand</label
						>
						<select
							id="model-brand"
							class="w-full px-3 py-2 rounded-lg border text-sm focus:outline-none focus:border-[#4299e1]"
					...
					<div class="setting-group">
						<label
							for="model-provider"
							class="block text-sm font-medium mb-2"
							style:color={textPrimary}>Provider</label
						>
						<select
							id="model-provider"
							class="w-full px-3 py-2 rounded-lg border text-sm focus:outline-none focus:border-[#4299e1]"
					...
					<div class="setting-group col-span-2">
						<label
							for="model-category"
							class="block text-sm font-medium mb-2"
							style:color={textPrimary}>Category</label
						>
						<select
							id="model-category"
							class="w-full px-3 py-2 rounded-lg border text-sm focus:outline-none focus:border-[#4299e1]"
				...
				<div class="setting-group">
					<label
						for="model-context"
						class="block text-sm font-medium mb-2"
						style:color={textPrimary}>Context Window</label
					>
					<input
						id="model-context"
						type="number"
				...
				<div class="setting-group">
					<span
						class="block text-sm font-medium mb-2"
						style:color={textPrimary}>Price per 1M tokens</span
					>
					<div class="grid grid-cols-2 gap-4">
						<div>
							<label
								for="model-price-input"
								class="text-xs"
								style:color={textSecondary}>Input</label
							>
							<div class="relative">
								<DollarSign
									size={14}
						...
						<div>
							<label
								for="model-price-output"
								class="text-xs"
								style:color={textSecondary}>Output</label
							>
							<div class="relative">
								<DollarSign
									size={14}
				...
				<div class="setting-group">
					<span
						class="block text-sm font-medium mb-2"
						style:color={textPrimary}>Capabilities</span
					>
					<div class="flex flex-wrap gap-2">
						{#each capabilities as cap}
							<button
								class="px-3 py-1.5 rounded-lg text-xs font-medium border transition-colors"
								style:border-color={formData.capabilities?.includes(
									cap,
								)
									? "#4299e1"
									: border}
								style:background-color={formData.capabilities?.includes(
									cap,
								)
									? "rgba(66, 153, 225, 0.2)"
									: inputBg}
								style:color={formData.capabilities?.includes(
									cap,
								)
									? "#4299e1"
									: textSecondary}
