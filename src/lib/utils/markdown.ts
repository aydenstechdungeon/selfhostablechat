...
renderer.codespan = ({ text }: { text: string }) => {
  return `<code class="px-1.5 py-0.5 rounded bg-[#2d3748] text-[#e2e8f0] text-sm font-mono">${text}</code>`;
};
...
renderer.table = (token: any) => {
  ...
  const headerHtml = header.map((cell: string) => `<th class="px-4 py-3 text-left text-sm font-semibold bg-[#0f1419] text-[#e2e8f0] border-b border-[#2d3748]">${cell}</th>`).join('');
  const rowsHtml = rows.map((row: string[], i: number) => {
    const isLastRow = i === rows.length - 1;
    const cells = row.map((cell: string) => `<td class="px-4 py-3 text-sm text-[#e2e8f0] ${isLastRow ? '' : 'border-b border-[#2d3748]/50'}">${cell}</td>`).join('');
...
export function parseMarkdown(content: string): string {
  ...
  try {
    ...
    // SECURITY: Removed 'onclick' from ALLOWED_ATTR to prevent XSS
    const sanitizedHtml = DOMPurify.sanitize(processedHtml, {
      ALLOWED_TAGS: [
        ...
        'div', 'span', 'button', 'svg', 'path', 'rect', 'polyline', 'line', 'circle', 'ellipse', 'polygon'
      ],
      ALLOWED_ATTR: [
        'href', 'title', 'target', 'rel',
        'src', 'alt',
        'class', 'data-code', 'data-lang', 'data-initialized', 'id',
        'xmlns', 'width', 'height', 'viewBox', 'fill', 'stroke', 'stroke-width', 'stroke-linecap', 'stroke-linejoin',
        'd', 'x', 'y', 'x1', 'y1', 'x2', 'y2', 'rx', 'ry', 'cx', 'cy', 'r', 'points'
        // SECURITY: Removed 'onclick' - use event delegation instead
      ]
    });
    
    // Cache the result
    setCachedMarkdown(content, sanitizedHtml);
